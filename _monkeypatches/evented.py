...
It will initialize the `smarterp.evented` variable.
"""
import smarterp
import sys

smarterp.evented = False


def patch_evented():
    if smarterp.evented or not (len(sys.argv) > 1 and sys.argv[1] == 'gevent'):
        return
    ...
    def gevent_wait_callback(conn, timeout=None):
        ...
        # under the BSD license (see in smarterp/debian/copyright)
        while 1:
            state = conn.poll()
