...
from .suite import SmarterpSuite
from .result import SmarterpTestResult
...
def get_test_modules(module):
    ...
    results = _get_tests_modules(importlib.util.find_spec(f'smarterp.addons.{module}'))
...
def _get_upgrade_test_modules(module):
    upgrade_modules = (
        f"smarterp.upgrade.{module}",
        f"smarterp.addons.{module}.migrations",
        f"smarterp.addons.{module}.upgrades",
    )
...
def make_suite(module_names, position='at_install'):
    ...
    return SmarterpSuite(sorted(tests, key=lambda t: t.test_sequence))
...
def run_suite(suite, global_report=None):
    ...
    results = SmarterpTestResult(global_report=global_report)
    suite(results)
