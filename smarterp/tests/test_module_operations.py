...
import smarterp
from smarterp.tools import config, topological_sort, unique
from smarterp.modules.registry import Registry
from smarterp.netsvc import init_logger
from smarterp.tests import standalone_tests
import smarterp.tests.loader

_logger = logging.getLogger('smarterp.tests.test_module_operations')
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Script for testing the install / uninstall / reinstall"
                    " cycle of Smarterp modules. Prefer the 'cycle' subcommand to"
    ...
    parser.add_argument("--data-dir", "-D", dest="data_dir", type=str,
        help="Directory where to store Smarterp data"
    )
    ...
    parser.add_argument("--addons-path", "-p", type=str, action=CheckAddons,
        help="Comma-separated list of paths to directories containing extra Smarterp modules")
...
def test_standalone(args):
    ...
    for module_name in registry._init_modules:
        ...
        smarterp.tests.loader.get_test_modules(module_name)
...
if __name__ == '__main__':
    ...
    # handle paths option
    if args.addons_path:
        smarterp.tools.config['addons_path'] = ','.join([args.addons_path, smarterp.tools.config['addons_path']])
        if args.data_dir:
            smarterp.tools.config['data_dir'] = args.data_dir
        smarterp.modules.module.initialize_sys_path()
    ...
    logging.config.dictConfig({
        ...
        'loggers': {
            'smarterp.modules.loading': {'level': 'CRITICAL'},
            'smarterp.sql_db': {'level': 'CRITICAL'},
            'smarterp.models.unlink': {'level': 'WARNING'},
            'smarterp.addons.base.models.ir_model': {'level': "WARNING"},
        }
    })
