...
SMARTERP_TEST_MAX_FAILED_TESTS = max(1, int(os.environ.get('SMARTERP_TEST_MAX_FAILED_TESTS', sys.maxsize)))

stats_logger = logging.getLogger('smarterp.tests.stats')
...
class SmarterpTestResult(object):
    """
    ...
    def _checkShouldStop(self):
        if self.total_errors_count() >= SMARTERP_TEST_MAX_FAILED_TESTS:
            ...
            if not global_report.shouldStop:
                _logger.error(
                    ...
                    "Remaining tests will be skipped.", SMARTERP_TEST_MAX_FAILED_TESTS)
