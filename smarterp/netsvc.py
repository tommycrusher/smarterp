...
def log(logger, level, prefix, msg, depth=None):
    warnings.warn(
        "smarterp.netsvc.log is deprecated starting Smarterp 18, use normal logging APIs",
...
class PostgreSQLHandler(logging.Handler):
    ...
    def __init__(self):
        ...
        if tools.config['log_db'] != '%d':
            with contextlib.suppress(Exception), tools.mute_logger('smarterp.sql_db'), sql_db.db_connect(tools.config['log_db'], allow_uri=True).cursor() as cr:
    ...
    def emit(self, record):
        ...
        if not dbname:
            return
        with contextlib.suppress(Exception), tools.mute_logger('smarterp.sql_db'), sql_db.db_connect(dbname, allow_uri=True).cursor() as cr:
...
def init_logger():
    ...
    warnings.filterwarnings('ignore', category=BytesWarning, module='smarterp.tools.image')
    ...
    if os.name == 'posix' and isinstance(handler, logging.StreamHandler) and (is_a_tty(handler.stream) or os.environ.get("SMARTERP_PY_COLORS")):
...
DEFAULT_LOG_CONFIGURATION = [
    'smarterp.http.rpc.request:INFO',
    'smarterp.http.rpc.response:INFO',
    ':INFO',
]
PSEUDOCONFIG_MAPPER = {
    'debug_rpc_answer': ['smarterp:DEBUG', 'smarterp.sql_db:INFO', 'smarterp.http.rpc:DEBUG'],
    'debug_rpc': ['smarterp:DEBUG', 'smarterp.sql_db:INFO', 'smarterp.http.rpc.request:DEBUG'],
    'debug': ['smarterp:DEBUG', 'smarterp.sql_db:INFO'],
    'debug_sql': ['smarterp.sql_db:DEBUG'],
    'info': [],
    'runbot': ['smarterp:RUNBOT', 'werkzeug:WARNING'],
    'warn': ['smarterp:WARNING', 'werkzeug:WARNING'],
    'error': ['smarterp:ERROR', 'werkzeug:ERROR'],
    'critical': ['smarterp:CRITICAL', 'werkzeug:CRITICAL'],
}
