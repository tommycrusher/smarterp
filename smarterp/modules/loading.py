...
import smarterp
import smarterp.modules.db
import smarterp.modules.graph
import smarterp.modules.migration
import smarterp.modules.registry
...
def force_demo(env):
    ...
    """
    graph = smarterp.modules.graph.Graph()
...
def load_module_graph(env, graph, status=None, perform_checks=True,
    ...
    migrations = smarterp.modules.migration.MigrationManager(env.cr, graph)
    module_count = len(graph)
    _logger.info('loading %d modules...', module_count)

    # register, instantiate and initialize models for each modules
    t0 = time.time()
    loading_extra_query_count = smarterp.sql_db.sql_counter
    ...
    for index, package in enumerate(graph, 1):
        ...
        module_extra_query_count = smarterp.sql_db.sql_counter
        ...
        if new_install:
            py_module = sys.modules['smarterp.addons.%s' % (module_name,)]
        ...
        if needs_update:
            ...
            overwrite = smarterp.tools.config["overwrite_existing_translations"]
        ...
        if tools.config.options['test_enable'] and (needs_update or not updating):
            ...
            if suite.countTestCases():
                if not needs_update:
                    registry.setup_models(env.cr)
                # Python tests
                tests_t0, tests_q0 = time.time(), smarterp.sql_db.sql_counter
                test_results = loader.run_suite(suite, global_report=report)
                report.update(test_results)
                test_time = time.time() - tests_t0
                test_queries = smarterp.sql_db.sql_counter - tests_q0
        ...
        extra_queries = smarterp.sql_db.sql_counter - module_extra_query_count - test_queries
        extras = []
        if test_queries:
    ...
    _logger.runbot("%s modules loaded in %.2fs, %s queries (+%s extra)",
                   ...
                   smarterp.sql_db.sql_counter - loading_extra_query_count)  # extra queries: testes, notify, any other closed cursor
...
def load_modules(registry, force_demo=False, status=None, update_module=False):
    ...
    with registry.cursor() as cr:
        ...
        if not smarterp.modules.db.is_initialized(cr):
        ...
        if update_module and tools.config['update']:
            for pyfile in tools.config['pre_upgrade_scripts'].split(','):
                smarterp.modules.migration.exec_script(cr, graph['base'].installed_version, pyfile, 'base', 'pre')

        if update_module and smarterp.tools.sql.table_exists(cr, 'ir_model_fields'):
            ...
            if smarterp.tools.sql.column_exists(cr, 'ir_model_fields', 'company_dependent'):
        ...
        migrations = smarterp.modules.migration.MigrationManager(cr, graph)
        for package in graph:
        ...
        if update_module:
            ...
            if modules_to_remove:
                ...
                for pkg in pkgs:
                    ...
                    if uninstall_hook:
                        py_module = sys.modules['smarterp.addons.%s' % (pkg.name,)]
                ...
                registry = smarterp.modules.registry.Registry.new(
