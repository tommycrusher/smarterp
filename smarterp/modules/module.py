...
import smarterp
import smarterp.tools as tools
import smarterp.release as release
from smarterp.tools.misc import file_path
...
class UpgradeHook(object):
    """Makes the legacy `migrations` package being `smarterp.upgrade`"""

    def find_spec(self, fullname, path=None, target=None):
        if re.match(r"^smarterp\.addons\.base\.maintenance\.migrations\b", fullname):
    ...
    def load_module(self, name):
        ...
        canonical_upgrade = name.replace("smarterp.addons.base.maintenance.migrations", "smarterp.upgrade")
...
def initialize_sys_path():
    """
    Setup the addons path ``smarterp.addons.__path__`` with various defaults
    and explicit directories.
    """
    # hook smarterp.addons on data dir
    dd = os.path.normcase(tools.config.addons_data_dir)
    if os.access(dd, os.R_OK) and dd not in smarterp.addons.__path__:
        smarterp.addons.__path__.append(dd)

    # hook smarterp.addons on addons paths
    for ad in tools.config['addons_path'].split(','):
        ad = os.path.normcase(os.path.abspath(ad.strip()))
        if ad not in smarterp.addons.__path__:
            smarterp.addons.__path__.append(ad)

    # hook smarterp.addons on base module path
    base_path = os.path.normcase(os.path.abspath(os.path.join(os.path.dirname(os.path.dirname(__file__)), 'addons')))
    if base_path not in smarterp.addons.__path__ and os.path.isdir(base_path):
        smarterp.addons.__path__.append(base_path)

    # hook smarterp.upgrade on upgrade-path
    from smarterp import upgrade
    ...
    # create decrecated module alias from smarterp.addons.base.maintenance.migrations to smarterp.upgrade
    spec = importlib.machinery.ModuleSpec("smarterp.addons.base.maintenance", None, is_package=True)
    maintenance_pkg = importlib.util.module_from_spec(spec)
    maintenance_pkg.migrations = upgrade
    sys.modules["smarterp.addons.base.maintenance"] = maintenance_pkg
    sys.modules["smarterp.addons.base.maintenance.migrations"] = upgrade

    # hook deprecated module alias from openerp to smarterp and "crm"-like to smarterp.addons
...
def get_module_path(module, downloaded=False, display_warning=True):
    ...
    if re.search(r"[\/\\]", module):
        return False
    for adp in smarterp.addons.__path__:
...
def get_resource_from_path(path):
    ...
    """
    resource = False
    sorted_paths = sorted(smarterp.addons.__path__, key=len, reverse=True)
...
def load_openerp_module(module_name):
    ...
    qualname = f'smarterp.addons.{module_name}'
...
def get_modules():
    ...
    plist = []
    for ad in smarterp.addons.__path__:
