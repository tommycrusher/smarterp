...
import smarterp
from smarterp.modules.db import FunctionStatus
from .. import SUPERUSER_ID
from smarterp.sql_db import TestCursor
from smarterp.tools import (
    config, lazy_classproperty,
    lazy_property, sql, OrderedSet, SQL,
    remove_accents,
)
from smarterp.tools.func import locked
from smarterp.tools.lru import LRU
from smarterp.tools.misc import Collector, format_frame

if typing.TYPE_CHECKING:
    from smarterp.models import BaseModel


_logger = logging.getLogger(__name__)
_schema = logging.getLogger('smarterp.schema')
...
class Registry(Mapping):
    ...
    def new(cls, db_name, force_demo=False, status=None, update_module=False):
        ...
        try:
            ...
            try:
                smarterp.modules.load_modules(registry, force_demo, status, update_module)
            except Exception:
                smarterp.modules.reset_modules_state(db_name)
                raise
        except Exception:
    ...
    def init(self, db_name):
        ...
        if config['test_enable'] or config['test_file']:
            from smarterp.tests.result import smarterpTestResult  # noqa: PLC0415
            self._assertion_report = smarterpTestResult()
        else:
        ...
        self._db = smarterp.sql_db.db_connect(db_name, readonly=False)
        ...
        if config['db_replica_host'] is not False or config['test_enable']:  # by default, only use readonly pool if we have a db_replica_host defined. Allows to have an empty replica host for testing
            self._db_readonly = smarterp.sql_db.db_connect(db_name, readonly=True)
        ...
        with closing(self.cursor()) as cr:
            self.has_unaccent = smarterp.modules.db.has_unaccent(cr)
            self.has_trigram = smarterp.modules.db.has_trigram(cr)
    ...
    def init_models(self, cr, model_names, context, install=True):
        ...
        env = smarterp.api.Environment(cr, SUPERUSER_ID, context)
    ...
    def cursor(self, /, readonly=False):
        ...
        if self.test_cr is not None:
            ...
            return TestCursor(self.test_cr, self.test_lock, readonly and self.test_readonly_enabled, current_test=smarterp.modules.module.current_test)
