...
r"""\
Smarterp HTTP layer / WSGI application
...
:class:`~smarterp.http.Controller`: class and its companion the
:func:`~smarterp.http.route`: method decorator. Together they are used to
...
Application.__call__
  ...
  request and itself in an Smarterp http request. The Smarterp http request is
...
Smarterp URLs are CSRF-protected by default (when accessed with unsafe
HTTP methods). See
https://www.smartbygg.no/documentation/master/developer/reference/addons/http.html#csrf
for more details.

* if this endpoint is accessed through Smarterp via py-QWeb form, embed a CSRF
...
def db_list(force=False, host=None):
    ...
    :param bool force: See :func:`~smarterp.service.db.list_dbs`:
    ...
    """
    try:
        dbs = smarterp.service.db.list_dbs(force)
...
def db_filter(dbs, host=None):
    ...
    if config['db_name']:
        # In case --db-filter is not provided and --database is passed, Smarterp will
...
def dispatch_rpc(service_name, method, params):
    ...
    """
    rpc_dispatchers = {
        'common': smarterp.service.common.dispatch,
        'db': smarterp.service.db.dispatch,
        'object': smarterp.service.model.dispatch,
    }
...
class Stream:
    ...
    def from_path(cls, path, filter_ext=('',), public=False):
        ...
        :param path: See :func:`~smarterp.tools.file_path`
        :param filter_ext: See :func:`~smarterp.tools.file_path`
    ...
    def get_response(
        ...
        **send_file_kwargs
        ...
        :param send_file_kwargs: Other keyword arguments to send to
            :func:`smarterp.tools._vendor.send_file.send_file` instead of
...
class Controller:
    ...
    :class:`~smarterp.http.Controller` can use the :func:`~smarterp.http.route`:
    ...
    :func:`~smarterp.http.route`:. Please note that the decorators of all
    ...
    .. code-block:
        ...
        class GreetingController(smarterp.http.Controller):
    ...
    def __init_subclass__(cls):
        ...
        if Controller in cls.__bases__:
            ...
            module = path[2] if path[:2] == ['smarterp', 'addons'] else ''
...
def route(route=None, **routing):
    ...
    .. warning::
        ...
        :class:`~smarterp.http.Controller` for more details.
...
def _generate_routing_rules(modules, nodb_only, converters=None):
    ...
    def is_valid(cls):
        ...
        path = cls.__module__.split('.')
        return path[:2] == ['smarterp', 'addons'] and path[2] in modules
    ...
    def build_controllers():
        ...
        installed modules). Modules in this context are smarterp addons.
        """
        # Controllers defined outside of smarterp addons are outside of the
        # controller inheritance/extension mechanism.
        yield from (ctrl() for ctrl in Controller.children_classes.get('', []))

        # Controllers defined inside of smarterp addons can be extended in
...
class Session(collections.abc.MutableMapping):
    ...
    def authenticate(self, dbname, credential):
        ...
        with registry.cursor() as cr:
            env = smarterp.api.Environment(cr, pre_uid, {})
        ...
        if request and request.session is self and request.db == dbname:
            request.env = smarterp.api.Environment(request.env.cr, self.uid, self.context)
...
class GeoIP(collections.abc.Mapping):
    ...
    .. note:
        ...
        :attr:`~smarterp.http.request.geoip`.

    .. code-block:

        >>> GeoIP('127.0.0.1').country.iso_code
        >>> smarterp_ip = socket.gethostbyname('smartbygg.no')
        >>> GeoIP(smarterp_ip).country.iso_code
        'FR'
    """
...
class _Response(werkzeug.wrappers.Response):
    ...
    def load(cls, result, fname="<function>"):
        ...
        :returns: The created :class:`~smarterp.http.Response`.
        :rtype: Response
...
class Response(Proxy):
    ...
    # smarterp.http._response attributes
    load = ProxyFunc()
...
class Request:
    ...
    def update_env(self, user=None, context=None, su=None):
        ...
        :type user: int or :class:`res.users record<~smarterp.addons.base.models.res_users.Users>`
    ...
    def make_response(self, data, headers=None, cookies=None, status=200):
        ...
        :rtype: :class:`~smarterp.http.Response`
        """
    ...
    def _serve_db(self):
        ...
        try:
            ...
            self.env = smarterp.api.Environment(cr_readwrite, self.session.uid, self.session.context)
            try:
            ...
            with contextlib.closing(self.registry.cursor(readonly=readonly_cr)) as cr:
                ...
                try:
                    ...
                    if isinstance(exc, HTTPException) and exc.code is None:
                        raise  # bubble up to smarterp.http.Application.__call__
...
class HttpDispatcher(Dispatcher):
    ...
    def dispatch(self, endpoint, args):
        ...
        See :meth:`~smarterp.http.Response.load` method for the compatible
...
class JsonRPCDispatcher(Dispatcher):
    ...
    def handle_error(self, exc: Exception) -> collections.abc.Callable:
        ...
        if isinstance(exc, NotFound):
            ...
            error['code'] = 100
            error['message'] = "Smarterp Session Expired"
...
class Application:
    ...
    def statics(self):
        ...
        """
        mod2path = {}
        for addons_path in smarterp.addons.__path__:
    ...
    def nodb_routing_map(self):
        ...
        for url, endpoint in _generate_routing_rules([''] + smarterp.conf.server_wide_modules, nodb_only=True):
    ...
    def session_store(self):
        path = smarterp.tools.config.session_dir
    ...
    def __call__(self, environ, start_response):
        ...
        if smarterp.tools.config['proxy_mode'] and environ.get("HTTP_X_FORWARDED_HOST"):
            ...
            try:
                ...
                if self.get_static_file(httprequest.path):
                    ...
                    try:
                        ...
                        if (httprequest.path.startswith('/smarterp/')
                            or httprequest.path in (
                                '/smarterp', '/web', '/web/login', '/test_http/ensure_db',
                            )):
