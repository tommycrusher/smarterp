...
import smarterp
from smarterp.modules import get_modules
from smarterp.modules.registry import Registry
from smarterp.release import nt_service_name
from smarterp.tools import config
from smarterp.tools.cache import log_ormcache_stats
from smarterp.tools.misc import stripped_sys_argv, dumpstacks
...
def set_limit_memory_hard():
    ...
    if smarterp.evented and config['limit_memory_hard_gevent']:
...
class RequestHandler(werkzeug.serving.WSGIRequestHandler):
    ...
    def setup(self):
        ...
        me.name = 'smarterp.service.http.request.%s' % (me.ident,)
...
class ThreadedWSGIServerReloadable(LoggingBaseWSGIServerMixIn, werkzeug.serving.ThreadedWSGIServer):
    ...
    def __init__(self, host, port, app):
        # The smarterp_MAX_HTTP_THREADS environment variable allows to limit the amount of concurrent
        ...
        self.max_http_threads = os.environ.get("smarterp_MAX_HTTP_THREADS")
...
class FSWatcherWatchdog(FSWatcherBase):
    def __init__(self):
        ...
        for path in smarterp.addons.__path__:
...
class FSWatcherInotify(FSWatcherBase):
    ...
    def start(self):
        self.started = True
        self.thread = threading.Thread(target=self.run, name="smarterp.service.autoreload.watcher")
...
class ThreadedServer(CommonServer):
    ...
    def cron_thread(self, number):
        ...
        from smarterp.addons.base.models.ir_cron import ir_cron
        def _run_cron(cr):
            pg_conn = cr._cnx
            ...
            while config['limit_time_worker_cron'] <= 0 or (time.monotonic() - alive_time) <= config['limit_time_worker_cron']:
                ...
                registries = smarterp.modules.registry.Registry.registries
        ...
        while True:
            conn = smarterp.sql_db.db_connect('postgres')
    ...
    def cron_spawn(self):
        ...
        for i in range(smarterp.tools.config['max_cron_threads']):
            def target():
                self.cron_thread(i)
            t = threading.Thread(target=target, name="smarterp.service.cron.cron%d" % i)
            t.daemon = True
            t.type = 'cron'
            t.start()
    ...
    def run(self, preload=None, stop=False):
        ...
        if stop:
            if config['test_enable']:
                from smarterp.tests.result import _logger as logger  # noqa: PLC0415
...
class Worker(object):
    ...
    def setproctitle(self, title=""):
        setproctitle('smarterp: %s %s %s' % (self.__class__.__name__, self.pid, title))
    ...
    def run(self):
        try:
            ...
            _logger.info("Worker (%s) exiting. request_count: %s, registry count: %s.",
                         ...
                         len(smarterp.modules.registry.Registry.registries))
            self.stop()
        except Exception:
...
class WorkerHTTP(Worker):
    ...
    def __init__(self, multi):
        ...
        # The smarterp_HTTP_SOCKET_TIMEOUT environment variable allows to control socket timeout for
        ...
        sock_timeout = os.environ.get("smarterp_HTTP_SOCKET_TIMEOUT")
...
class WorkerCron(Worker):
    ...
    def _db_list(self):
        if config['db_name']:
            ...
            db_names = smarterp.service.db.list_dbs(True)
        return db_names
    ...
    def process_work(self):
        ...
        if len(db_names):
            ...
            from smarterp.addons import base
    ...
    def start(self):
        ...
        dbconn = smarterp.sql_db.db_connect('postgres')
...
def _reexec(updated_modules=None):
    ...
    if smarterp.tools.osutil.is_running_as_nt_service():
...
def load_test_file_py(registry, test_file):
    ...
    from smarterp.tests import loader  # noqa: PLC0415
    from smarterp.tests.suite import smarterpSuite  # noqa: PLC0415
    ...
    for dbname in dbnames:
        try:
            ...
            if config['test_enable']:
                from smarterp.tests import loader  # noqa: PLC0415
                t0 = time.time()
                t0_sql = smarterp.sql_db.sql_counter
                ...
                if post_install_suite.has_http_case():
                    with registry.cursor() as cr:
                        env = smarterp.api.Environment(cr, smarterp.SUPERUSER_ID, {})
                ...
                _logger.info("%d post-tests in %.2fs, %s queries",
                             ...
                             time.time() - t0,
                             smarterp.sql_db.sql_counter - t0_sql)
...
def start(preload=None, stop=False):
    """ Start the smarterp http server and cron processor.
    """
    global server
    ...
    if smarterp.evented:
        server = GeventServer(smarterp.http.root)
    elif config['workers']:
        if config['test_enable'] or config['test_file']:
            _logger.warning("Unit testing in workers mode could fail; use --workers 0.")

        server = PreforkServer(smarterp.http.root)
    else:
        if platform.system() == "Linux" and sys.maxsize > 2**32 and "MALLOC_ARENA_MAX" not in os.environ:
            ...
            # which smarterp is based upon in order to limit the memory usage for threaded workers.
        ...
        server = ThreadedServer(smarterp.http.root)

    watcher = None
    if 'reload' in config['dev_mode'] and not smarterp.evented:
        if inotify:
