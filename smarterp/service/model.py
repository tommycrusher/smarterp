...
import smarterp
from smarterp.exceptions import UserError, ValidationError, AccessError
from smarterp.models import BaseModel
from smarterp.http import request
from smarterp.modules.registry import Registry
from smarterp.tools import DotDict, lazy
from smarterp.tools.translate import translate_sql_constraint
...
def execute_cr(cr, uid, obj, method, *args, **kw):
    ...
    cr.reset()
    env = smarterp.api.Environment(cr, uid, {})
    recs = env.get(obj)
    if recs is None:
    ...
    result = retrying(partial(smarterp.api.call_kw, recs, method, args, kw), env)
...
def retrying(func, env):
    ...
    :param smarterp.api.Environment env: The environment where the registry
